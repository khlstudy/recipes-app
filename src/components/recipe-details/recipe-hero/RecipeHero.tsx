...
const RecipeHero = ({
  ...
  const handleTagClick = (tag: string) => {
    const params = new URLSearchParams({ search: tag, searchType: "tag" });
        ...
        <div className={styles["recipe-hero__actions"]}>
          ...
          {isAuthenticated && (
            <Button
              ...
              className={styles["recipe-hero__view-fav"]}>
              <Icon
                src={`${ICONS_PATH}${RECIPE_ICON_IDS.eye}`}
                size={18}
                className={styles["recipe-hero__fav-icon"]}
              />
              View favorites
          ...
          <ul className={styles["recipe-hero__tags"]}>
            {recipe.tags.map((tag) => (
              <li key={tag}>
                <button
                  type="button"
                  className={styles["recipe-hero__tag"]}
                  onClick={() => handleTagClick(tag)}
                  aria-label={`Search catalog for tag ${tag}`}>
